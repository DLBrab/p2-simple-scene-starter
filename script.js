...
function draw() {
  ...
  for (i = 0; i < height; i = i + distanceBetweenSpotsY) {
    for (j = width; j > 0; j = j - distanceBetweenSpotsX) {
      ...
      fill (iceLightColor)
      ellipse(j - offsetX / 3,i - offsetY / 3 + distanceBetweenSpotsY,rad - 2,rad - 2)
      fill (iceMidColor)
      ellipse(j - offsetX / 2,i - offsetY / 2 + distanceBetweenSpotsY,rad - 1,rad - 1)
      fill (iceDarkColor)
      ellipse(j - offsetX,i - offsetY + distanceBetweenSpotsY,rad,rad)
    }
